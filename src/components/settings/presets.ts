...
export const presets: ColorVariants[] = [
  {
    name: 'default',
...
export const presetsMap: Record<ThemeColorPresetsValue, typeof defaultPreset> = {
...
export const presetsOption: PresetsOption[] = presets.map((color) => ({
  name: color.name,
  value: color.main,
}));

export function getPresets(key: ThemeColorPresetsValue) {
  return presetsMap[key] ?? presetsMap.default;
}
