...
function createShadow(color: string, preset: ThemeColorPresetsValue) {
  const transparent = alpha(color, 0.16);
  const themePalette = preset ? getPresets(preset) : palette('light').primary;
  return {
    ...
    //
    primary: `0 8px 16px 0 ${alpha(themePalette.main, 0.24)}`,
...
export default function customShadows(themeMode: 'light' | 'dark', preset: ThemeColorPresetsValue) {
  return themeMode === 'light' ? createShadow(LIGHT_MODE, preset) : createShadow(DARK_MODE, preset);
}
