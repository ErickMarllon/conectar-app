...
// @types
import type { IUserProfilePost } from '@/shared/interfaces/IUser';
// auth
// utils
...
export default function ProfilePostCard({ post }: Props) {
  ...
  return (
    <Card>
      <CardHeader
        disableTypography
        avatar={<CustomAvatar src={user?.avatar_url} alt={displayName} name={displayName} />}
        title={
      ...
      >
        ...
        <CustomAvatarGroup>
          {post.personLikes.map((person) => (
            <CustomAvatar key={person.name} alt={person.name} src={person.avatar_url} />
          ))}
        </CustomAvatarGroup>
        ...
        <Stack spacing={1.5} sx={{ px: 3, pb: 2 }}>
          {post.comments.map((comment) => (
            <Stack key={comment.id} direction="row" spacing={2}>
              <CustomAvatar alt={comment.author.name} src={comment.author.avatar_url} />
